...
if ($path === null) {
    fwrite(STDERR, "Usage: php scripts/perf/request-timeline.php <base-url> <db-name> <admin-email> <password> <path> [runs]\n");
    exit(1);
}
...
perf_request($base . '/login/password', $jar, json_encode(['_csrf_token' => $m[1] ?? '', 'email' => $email, 'password' => $password, 'rgpd_consent' => true]));

$pdo = new PDO(
    sprintf('mysql:host=%s;port=%d;dbname=%s', getenv('PERF_DB_HOST') ?: (getenv('TEST_DB_HOST') ?: '127.0.0.1'), (int) (getenv('PERF_DB_PORT') ?: (getenv('TEST_DB_PORT') ?: 3306)), $db),
...
for ($run = 0; $run < $runs; $run++) {
    ...
    $row = $pdo->query("SELECT * FROM event_log WHERE id > $lastId AND event_type = 'debug_request_timeline' ORDER BY id DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
    if ($row === false) {
