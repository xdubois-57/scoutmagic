...
# ---------------------------------------------------------------------
# Static asset lifetimes. Safe because every template reference to
# /assets/** goes through the asset() helper, whose ?v=… changes with
# the release (pinned by Tests\Core\View\AssetVersioningTest) — a
# year-long cache can never serve a stale script against a new page.
# Two shorter tiers for what is referenced WITHOUT a version: fonts and
# images reached from inside vendored CSS files keep their bare URL, so
# they get days, not a year — stale for at most that long after a
# vendored-library upgrade. The service worker script is exempt by the
# platform itself (browsers bypass the HTTP cache for SW updates).
#
# The type list is pinned between the two .htaccess sources by
# Tests\Bootstrap\HtaccessExpiresTest. mod_expires is not universal on
# shared hosting, so this degrades silently when absent, like the rest
# of this file.
# ---------------------------------------------------------------------
