...
`parents` are ancestor labels naming a *menu* (`Core\View\MenuBuilder`'s own `label`, e.g. `"Espace des chefs"`) — declared as plain strings, never a URL, since a menu category has no landing page of its own to hardcode (most don't have one at all). `partials/breadcrumb_bar.html.twig` decides at render time whether a parent is clickable, by matching it against the same `menus` global the nav renders from and looking for that menu's first page that is neither a separator, a dynamic per-member entry, nor a `#` placeholder (Espace des animés' empty-state row) — skipping past all of those, not just the very first entry. If one exists and isn't the page already being viewed (never a self-link — e.g. Espace des chefs' only current page is Staffs itself, so its own breadcrumb never links that parent), it renders as `<a>`; otherwise the parent stays plain text, exactly as if no link were possible — never an invented URL like `/` or `#`. This is a template-side lookup only: the breadcrumb declaration itself never carries a parent URL.
