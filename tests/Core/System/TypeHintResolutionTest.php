...
class TypeHintResolutionTest extends TestCase
{
    ...
    {
        if (!self::exists($class) && !trait_exists($class)) {
            // A file whose class name does not match its path is a separate
            // concern (Composer's own PSR-4 check), not this test's.
            self::markTestSkipped("{$class} is not autoloadable under its own path");
        }
