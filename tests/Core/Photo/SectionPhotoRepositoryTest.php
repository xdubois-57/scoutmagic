...
class SectionPhotoRepositoryTest extends TestCase
{
    private \PDO $pdo;
    private SectionPhotoRepository $repository;
    ...
    protected function setUp(): void
    {
        ...
        $this->repository = new SectionPhotoRepository($this->pdo);
    ...
    private function createFile(string $name): int
    {
        ...
        $stmt->execute(['core/section_photos/' . $name, $name]);
        return (int) $this->pdo->lastInsertId();
    }

    public function testFindFileIdReturnsNullWhenNoPhoto(): void
    {
        $result = $this->repository->findFileIdForYearOrEarlier($this->sectionId, $this->yearNewId);
    ...
    {
        $this->repository->upsert($this->sectionId, $this->yearNewId, $this->fileIdA, null);
    ...
    {
        $this->repository->upsert($this->sectionId, $this->yearOldId, $this->fileIdA, null);
        $this->repository->upsert($this->sectionId, $this->yearMidId, $this->fileIdB, null);
    ...
    public function testFindFileIdIgnoresFutureYearPhotos(): void
    {
        ...
        $result = $this->repository->findFileIdForYearOrEarlier($this->sectionId, $this->yearOldId);
    ...
    {
        $this->repository->upsert($this->sectionId, $this->yearNewId, $this->fileIdA, null);
        $this->repository->upsert($this->sectionId, $this->yearNewId, $this->fileIdB, null);

        $result = $this->repository->findFileIdForYearOrEarlier($this->sectionId, $this->yearNewId);

        $this->assertSame($this->fileIdB, $result);

        $count = (int) $this->pdo->query('SELECT COUNT(*) FROM section_staff_photos')->fetchColumn();
