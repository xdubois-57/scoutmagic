 ...
 * is recorded against the session's account, so a visitor with no account
 * has nothing to record. The guard is the Router's, never the
 * controller's, so a stub is what proves it.
 */
final class PushInvitationRbacTest extends TestCase
{
    private const PATH = '/api/notifications/invitation';
    ...
    public function testAChiefIsAllowedToo(): void
    {
        $this->startTestSession();
     ...
     */
    public function testAPublicVisitorIsRefused(): void
    {
        $this->startTestSession();
     ...
     * session at all.
     */
    public function testASessionWhoseRoleIsPublicIsRefusedToo(): void
    {
        $this->startTestSession();
    ...
    {
        $router = new Router();
        $router->addRoute('POST', self::PATH, PushInvitationStubController::class, 'answer', 'identified');

        $frontController = new FrontController($router, $this->twig, $this->config);
        $frontController->registerController(
            PushInvitationStubController::class,
            new PushInvitationStubController($this->twig)
        );
...
class PushInvitationStubController extends AbstractController
{
    /**
