...
class TransactionAttachmentRepository
{
    ...
    /**
     * The earliest (lowest id) attachment linked to each transaction —
     * "the first receipt" for Service\MovementPresenter's counterparty/
     * description derivation. One query for every row on a page rather
     * than N (same precedent as countByTransactionIds()) — the join
     * table itself has no association-order column, so "first" is
     * approximated as "earliest-uploaded of the linked receipts".
     *
     * @param int[] $transactionIds
     * @return array<int, int> transaction_id => attachment_id
     */
    public function findFirstAttachmentIdsByTransactionIds(array $transactionIds): array
    {
        if ($transactionIds === []) {
            return [];
        }
        $placeholders = implode(',', array_fill(0, count($transactionIds), '?'));
        $stmt = $this->pdo->prepare(
            "SELECT transaction_id, MIN(attachment_id) AS attachment_id FROM finance_transaction_attachments
             WHERE transaction_id IN ({$placeholders}) GROUP BY transaction_id"
        );
        $stmt->execute(array_values($transactionIds));
        ...
        $result = [];
        foreach ($stmt->fetchAll(\PDO::FETCH_ASSOC) as $row) {
            $result[(int) $row['transaction_id']] = (int) $row['attachment_id'];
        }
        return $result;
    }
