...
There is intentionally **no `--skip-sonar-gate` flag** — unlike the four gates above, do not add one, do not comment out the call in `release.sh`, and do not work around a real finding by dismissing it in SonarQube without genuine justification. If the gate blocks a release, fix the finding, or resolve/dismiss it in SonarQube Cloud with a real justification (same standard as a Dependabot alert, see below), then re-run. Test the gate's own logic with `scripts/check-sonar-release.test.sh` (mocked, no live API calls) rather than by manipulating real findings.
...
There is no equivalent flag for the SonarQube Cloud gate, for any reason, including an explicit user request for an urgent release — see § SonarQube Cloud release gate above.
