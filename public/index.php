...
if (Request::isPostTooLarge()) {
    ...
    echo '<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>Fichier trop volumineux</title></head>'
        ...
        . 'Réessayez avec un fichier plus petit, puis revenez en arrière dans votre navigateur pour ne pas perdre '
        . 'votre saisie.</p>'
        . '</body></html>';
    exit;
}
...
if (!$notificationsV2Migrated) {
    ...
    $preMigrationPdo->prepare(
        ...
        'Indique si le nettoyage ponctuel des anciennes notifications/souscriptions push (passage au format chiffré) '
            . 'a été effectué.',
        0, 999,
    ]);
}
...
<p>Merci de patienter, cette page se rechargera automatiquement une fois la
mise à jour terminée. Vous pouvez aussi fermer cet onglet : la mise à jour
reprendra à l'endroit où elle s'est arrêtée lors de votre prochaine
visite.</p>
...
if ($settingService->get('settings_migrated') !== '1') {
    ...
    if (
        !empty($secrets['mail_from_address'])
        && ($settingService->get('contact_email') === '' || $settingService->get('contact_email') === null)
    ) {
...
$twig->addGlobal(
    'offline_cache_staleness_days',
    (int) $settingService->get('offline_cache_staleness_days', null, '30')
);
...
// (§8.110): whether this account has already answered « Plus tard ».
...
// installed application offers once (ARCHITECTURE.md §8.110). The
...
// §8.110) — the same server-side shape as the tips dialog just above: the
...
// « Activer les notifications ? » (§8.110). Here rather than next to the
...
if ($isEnabled('finance')) {
    ...
    $financeBalanceService = new \Modules\Finance\Service\BalanceService(
