...
**The menu entry is « Supervision », and the three screens under it are tabs.** It read « Support » — the same label the unit's own support page carries — so the one installation that has both showed two identical lines under Configuration, in the same group, going to two different places. The unit's page exists everywhere and keeps the name; the receiver's, which exists on one installation, is the one that moved. « Tableau de bord », « Tickets » and « Correspondances » are sub-pages of that single entry, rendered by the site's shared sub-navigation component like every other module's, not by a second one of its own. Each of the three names the page it hangs under in its breadcrumb — an `ancestors` entry, which renders as a link back, and not a `parents` entry, which names a MENU and would render « Supervision » as inert text.
