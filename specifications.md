...
| Prévisions d'effectifs (module registration) | chief | Read-only projected headcount for next scout year, per section and unit-wide — see §19.1 |
| Présences (module) | chief | Qui était là, réunion par réunion : une feuille par évènement de calendrier de section, quatre états par animé, un commentaire chiffré, et le registre annuel de la section. **Une seule entrée de menu** — tout le reste du module s'atteint depuis cette page. Voir §43 |
| Organiser les covoiturages (module covoiturage) | chief | Create a carpool for an outing — one or several calendar events, or a section — its place and dates, then follow who rides in which car; editing and deleting stay open to the creator, the staffs of the sections concerned and the Staff d'U — see §45 |
...
| Gérer le téléphone d'urgence (module sos_staff) | admin | On-call duty roster (month grid), default forwarding number, live redirect status, scheduled redirection list |
...
| Formulaire d'inscription (module registration) | admin | See §17 — request management: capacities/year code (age brackets are read-only, federation-fixed, shared with the Statistiques module), year selector, capacity-verification table, request list (filter/search), "non rapprochées"/"non clôturées" encarts with bulk refuse/withdraw, and a per-request fiche (status transitions, section prévue, tarif, internal notes, acceptance/refusal emails, manual Desk linking) |
| Passages de branche (module registration) | admin | Split arriving families and promoted animés between sections ahead of next scout year — see §18.2. Chef d'unité only (not a per-section chief), since spreading arrivals across sections needs the whole unit at once |
...
| Biens à louer (module rental) | admin | Which assets exist and who runs each one — creating an asset, its general description, its managers, archiving it, and the accounting account its money is expected on. Everything that is a property of one asset is set in that asset's own managed space instead. See §22 |
...
**The four site actions are four cards.** The old « Données du site » heading was dropped: once the page grew, everything past the Desk half is site data.
...
4. **Ajouter à mes contacts** — the member's contact card (ARCHITECTURE.md §8.115), offered as a QR code to scan and as a `text/vcard` file to download, **both at once and with no device detection**. The dialog states in one sentence what the card holds and for which scout year, and promises nothing about what happens afterwards: `UID` and `REV` help a client recognise the same person, some honour them and others file a second entry, so no wording may suggest the card updates itself in an address book. The card carries the full name (never the totem alone), the unit and section, the year's function, every known e-mail address, both telephone numbers **unlabelled** — the screen says « Tél. parent 1 / 2 », the card says nothing, deliberately — every postal address, the portrait (file only, never the QR code) and the history of functions year by year, capped at five years in the QR code and complete in the file. It carries **none** of the disability, supplementary insurance, sex, birth date, Desk identifier, patrol, formation level or communication consents; the disability is health data and is the one this page puts one line away from the telephone numbers. **This button exists on this page only** — not on the trombinoscope, where it would give every identified member a way to collect the staff's contact details one by one. Every card produced is journaled with the member's identifier and nothing else.
...
| Correspondances Desk | A box at the top of the page names what the site **currently** cannot match to anything it knows — an unqualified function, or a branch the code sorts last. It is the site's present state and not a record of the last import: a function created months ago and never qualified is still listed, because it is still unresolved — with what the site did instead, how many records are affected, and where to fix each one; it disappears by itself once everything is resolved, and says that the labels are also reported to the maintainer when the daily report is enabled. Map Desk functions to site roles; rename sections, set section email, and toggle section visibility across the site. Per age branch: federation logo (uploaded, falls back to a shipped default per canonical branch, else nothing) and explanation link (defaults to the Les Scouts federation page), shown on the member page's branch card (§4.2). |
...
| Maintenance | Backups (on-demand + automatic, database-only/config-only/full, encrypted). **No archive carries the content of a declared storage location** — the gallery included — because a location has a lifecycle of its own; the page says so where the scope is chosen, and a full reset leaves those folders alone for the same reason (ARCHITECTURE.md §8.109). Plus the **portable backup**, the one archive that carries the site's encryption keys and can therefore be restored onto a *different* installation, kept one at a time and meant to be downloaded then deleted; update from GitHub releases (check/install with automatic rollback on failure); reset actions (settings to defaults, restore a backup — an uploaded portable archive included, with its passphrase — full reinstall) — each destructive action requires typing an exact confirmation word; and the **off-site destination**, chosen from the storage locations the site has declared so backups do not sleep on the server they protect — a Google Drive folder is declared like any other destination, in Configuration > Stockage, and this page only says which one the archives go to. |
...
| Supervision (module support_dashboard) | Present **only** on the ScoutMagic installation acting as statistics receiver — the module declares `"visible_when": ["statistics_receiver"]` and is filtered out of module discovery everywhere else. Table of the installations reporting in, with filters, free search, sort and pagination; five indicator cards and two current-state charts, all recomputed on the filtered set; a detail dialog carrying every metric, who registered the installation's domain (asked of the registry from the daily report, since a site cannot report that about itself) and the exact raw JSON of the last accepted report; XLSX export of the filtered set; manual deletion behind a confirmation; a monthly-history chart independent of all of the above; and a « Correspondances » screen listing the Desk values no installation's code recognises, derived from the reports held and announced once per value (§8.51ter). `superadmin` only. |
| Fréquentation (module usage_stats) | Deux questions et rien d'autre : **« le site sert-il ? »** — combien de comptes se sont connectés ce mois-ci, rapporté au nombre de comptes existants — et **« qu'est-ce qui sert ? »** — quelles pages et quels modules sont ouverts, et surtout lesquels ne le sont jamais, ce qui est ce qui permet de décider qu'on désactive un module que personne n'ouvre. Aucun suivi nominatif, aucun cookie, aucune adresse IP. Voir §42 |
...
| Photos (module gallery) | Quatre onglets. **Général** : albums externes autorisés, nombre maximum de médias par album, et l'**emplacement des nouveaux albums** — le libellé compte, il s'applique aux albums créés à partir de ce moment et les albums existants ne bougent pas. **Photos** et **Vidéos** : les limites de taille, de dimension et de durée. **Albums** : la migration d'un album d'un emplacement vers un autre, en arrière-plan, les albums délégués — ceux qu'un autre module possède — listés avec les autres parce qu'ils occupent une place réelle et n'apparaissent nulle part ailleurs. Les emplacements eux-mêmes se déclarent dans Configuration > Stockage et non ici. |
...
| Stockage | Où ce site écrit ses fichiers, et ce qu'il y a dessus. **Tableau de bord** : ce qui ne va pas d'abord — un emplacement en erreur est énoncé avec sa conséquence (« les galeries ne peuvent plus rien écrire »), pas seulement avec un badge ; puis qui utilise quel emplacement ; puis l'espace **par volume**, regroupé par périphérique de sorte que deux dossiers d'un même disque ne soient pas comptés deux fois, chaque volume disant laquelle des deux mesures il utilise — le quota déclaré, qui ne couvre que le volume principal, ou celle du système ; puis les emplacements distants, dont l'occupation reste inconnue quand le type de stockage ne sait pas la dire sans tout parcourir. **Emplacements** : une fiche par destination — état, cible, conséquences (Photos, Vidéos, Sauvegardes, Place restante, produites à partir des capacités déclarées et jamais affichées telles quelles), usages servis, test, modification, suppression. Un emplacement local peut viser un sous-dossier de `storage/` ou un chemin absolu (montage réseau, second disque) ; aucune archive ne reprend le contenu d'un emplacement et une réinitialisation complète ne l'efface pas, où qu'il soit. **Ce qui protège un emplacement est un autre emplacement** : sur sa fiche se déclarent une destination, un délai de grâce et une cadence, et une passe nocturne copie ce qui manque puis — seulement si son listage est allé au bout — marque ce que la source n'a plus. Marquer n'est pas supprimer : un fichier revenu perd son compte à rebours, et la suppression n'atteint que ce qu'une passe précédente avait marqué et dont le délai est écoulé. Au-delà d'une proportion de disparitions en une seule passe, la purge s'arrête et le dit. « Rapatrier depuis la copie » remet en place ce que la source a perdu — la réparation dont on a besoin après une restauration. Le tableau de bord liste les emplacements sans copie qui tourne, à côté de ceux en erreur. Voir ARCHITECTURE.md §8.110. Un dossier situé dans la racine web est refusé. Le test de santé écrit réellement un fichier témoin, le relit et le supprime, sous un délai maximal — un montage réseau peut figer, et une page de configuration qui se fige est la seule page qui permettrait de le réparer. **Un compte Google Drive se raccorde ici aussi** : la fiche porte l'identifiant du client OAuth, l'adresse de redirection à déclarer chez Google au caractère près, le compte raccordé, et l'avertissement sur l'écran de consentement — publié, faute de quoi Google retire l'autorisation au bout de sept jours sans rien dire. Voir ARCHITECTURE.md §8.104 et §8.107. |
...
| Mon compte | Name, surname. Password. Passkeys. Notification preferences (link). Cookie preferences (link). « Revoir les astuces » — repropose every discovery tip from the top, offered only once some have been seen. **« Synchroniser mes contacts »**, offert aux seuls `admin` et `superadmin` et menant à sa propre page (`/account/devices`, même plancher) : les identifiants avec lesquels un carnet d'adresses se synchronise — la liste, la création, la révocation. Le mot de passe d'un accès n'est montré **qu'une seule fois**, à sa création, et n'est jamais récupérable ensuite. La page énonce la vérité désagréable avant le bouton et non après : la copie descendue reste sur l'appareil, suit ses sauvegardes, et révoquer n'efface rien. Elle rappelle aussi que l'accès suit le rôle — un chef qui quitte le staff perd la synchronisation au passage suivant, sans révocation manuelle (ARCHITECTURE.md §8.117). Depuis IT-03 la page donne aussi **l'adresse à taper dans le carnet d'adresses** : l'**adresse complète** (`…/carddav/staff/`), donnée comme valeur du champ « Serveur » et répétée dans le panneau qui affiche le mot de passe — le domaine seul envoie le client vers `/.well-known/carddav`, que plusieurs hébergements servent eux-mêmes avant que PHP ne soit atteint, et la connexion n'aboutit alors jamais (sur iPhone : « Vérification » sans fin, mot de passe pourtant correct). La page dit pourquoi, et nomme le chemin iOS jusqu'au champ. Elle porte aussi un bouton « Vérifier que cet hébergement laisse passer la synchronisation » : certains hébergements mutualisés refusent `PROPFIND` et `REPORT` avant PHP, et le seul symptôme côté téléphone est « impossible de se connecter », impossible à distinguer d'un mot de passe erroné (§4.7, ARCHITECTURE.md §8.118). |
...
**The menu entry is « Supervision », and the three screens under it are tabs.** It read « Support » — the same label the unit's own support page carries — so the one installation that has both showed two identical lines under Configuration, in the same group, going to two different places. The unit's page exists everywhere and keeps the name; the receiver's, which exists on one installation, is the one that moved. « Tableau de bord », « Tickets » and « Correspondances » are sub-pages of that single entry, rendered by the site's shared sub-navigation component like every other module's, not by a second one of its own. « Tickets » and « Correspondances » each name the page they hang under in their breadcrumb — an `ancestors` entry, which renders as a link back, and not a `parents` entry, which names a MENU and would render « Supervision » as inert text. The dashboard is that page, so it declares its `Configuration` menu parent and no ancestor: nothing can be its own ancestor.
...
**« Correspondances » lists the Desk values no installation's code recognises** (§8.51ter) — a function nobody qualified, or a branch sorted last for want of a match. A Desk tariff outside the three household ones is deliberately **not** among them: §8.75 says reporting one would be a false positive on every unit, and this code cannot tell such a tariff from one of the three spelled unusually. The list is **derived on every read** from the reports already held: the receiver runs the same ScoutMagic as the senders, so it recognises a value by calling its own code, and a value a later version learns leaves the page by itself with nothing to delete. Nothing is catalogued centrally, on purpose — a table of « known values » would have to be kept in step with the code for ever, and would be wrong exactly when it mattered.

Each line shows the kind, the raw value as first spelled, how many installations report it and which, when it first appeared and when it was last seen, **the oldest version still reporting it**, the effect in French, and the hard-coded table to complete. The version column is not decoration: a value corrected in the code keeps being reported by everyone who has not upgraded, and without it the same value is picked up again next month by somebody who thinks it was forgotten. A **function** is the one kind with no table to complete — this code has no list of known functions, so no release can recognise one — and the page says so rather than promising a fix that cannot come.

**One notification per value, ever**, sent when a report arrives rather than when somebody opens the page: a page nobody opens announces nothing, which is the whole problem this answers. It announces whatever is still waiting to be announced, not merely what the report in hand brought in — a value first seen while nobody had subscribed to the type is a value nobody has been told about, and it is the next report that tells them. It is generic and points at the page, so nothing of a federation's vocabulary travels to a phone. « Écarter » is reversible and filtered, never a delete — the next report would recreate the row and the judgement would have to be made again every morning. Only three facts are stored (first sighting, notification sent, set aside); everything else is recomputed.
...
**Tout se signale sur GitHub, et la page le dit avant tout le reste.** Bugs comme demandes vont dans un signalement public — suivi, commenté, retrouvable et refermé devant tout le monde — plutôt que dans un guichet que deux personnes voient : l'unité d'à côté qui a le même défaut n'apprenait rien, et celle qui l'avait signalé le mois précédent était réinterrogée. La carte « Signaler un problème » porte le lien direct vers la page des signalements, est le seul bouton primaire de la page, et **s'affiche même quand le formulaire d'envoi est indisponible** : le formulaire est un complément, jamais la porte d'entrée. **Aucune adresse de support n'apparaît nulle part** — une adresse à côté d'un traqueur public rouvre le guichet privé et y attire précisément les gens les moins à l'aise avec l'outil public. Ce qui reste local est ce qu'un dépôt public ne peut pas porter : les preuves techniques. On les envoie, on obtient une **référence**, on cite la référence dans le signalement, et les **cinq derniers envois** restent affichés, chacun replié — une référence sert à être recopiée, et personne ne signale dans la minute.

**Envoyer des informations techniques est un seul geste.** Le bloc demande une catégorie, une description, une adresse de contact et **deux cases obligatoires** — l'archive de diagnostic, et un e-mail de test. Un clic envoie alors le ticket, l'archive et la sonde ; il n'y a plus de bouton séparé pour l'une ou l'autre, parce que trois gestes étaient trois façons de n'envoyer que la moitié de ce qu'il faut. Les deux cases sont **revérifiées côté serveur** : un attribut `required` est une décoration qu'un POST fabriqué à la main contourne. Le **détail** de ce que contient l'archive — quinze rubriques décrites en une phrase chacune — est replié par défaut : déplié, il poussait le bouton d'envoi hors de l'écran. Ce qui reste visible est l'objet du consentement (ce que c'est, combien il pèse, combien de rubriques), le détail s'ouvre d'un clic **au-dessus** de la case, et les deux cases restent hors du repli — accepter de transmettre « quelque chose » n'est pas accepter de transmettre ceci. Si aucune archive n'existe encore, elle est générée en tâche de fond et jointe automatiquement dès qu'elle est prête — « toujours transmise » sans trou dans la promesse. Rien de ce qui suit le ticket ne peut le défaire : une archive refusée ou un relais injoignable est une phrase de plus dans la confirmation, jamais un rapport perdu.
...
**Le receveur va chercher deux choses qu'une installation ne peut pas dire d'elle-même.** À l'arrivée d'un ticket, il relève les **enregistrements DNS** de l'hôte de l'installation et les conserve sur le ticket : la moitié des « le site ne répond plus » et des « les e-mails n'arrivent pas » est un enregistrement que la personne aura corrigé avant qu'on lise son ticket, donc c'est le relevé du moment qui vaut, pas celui du jour où un mainteneur ouvre la page. À la réception d'un rapport quotidien, il consulte le **WHOIS** du domaine et affiche l'enregistrement — registrar, dates, serveurs de noms — sur la fiche de l'installation : un site ne sait pas qui détient son propre nom, et « ce domaine expire dans trois semaines » arrive au support sous la forme « le site ne marche plus ». Une consultation par installation et par mois, jamais une par rapport : les registres limitent par adresse source, et se faire refuser est exactement la façon dont un diagnostic cesse de fonctionner sur le parc assez grand pour en avoir besoin. Les deux tournent **après** que la ligne qu'elles annotent est écrite et confirmée, et n'échouent jamais bruyamment — un ticket sans relevé reste un ticket. Le WHOIS **brut** peut nommer une personne physique : il est chiffré, ne s'affiche sur aucun écran, et ne ressort que dans le dossier d'un ticket, qu'on télécharge délibérément ; seuls les champs organisationnels sont analysés et affichés.
...
**The request form asks for what answering it needs, and nothing more.** Dates, head count, a name, an e-mail, a **phone** and the **purpose of the letting** are required — the last two in the browser *and* on the server, because a `required` attribute is a convenience for the visitor and never a guarantee to the unit. A manager answering a request needs a number to call when an arrival goes wrong and needs to know what the hall is for; a request giving neither costs the first two e-mails of every file. The **organisation stays optional**: a family letting the hall for a communion has none, and demanding one only makes them invent an answer. Billing details are not asked for at all (§22.6).

**The conditions a renter accepts are never empty.** They are per-asset rich text, and a standard Belgian body ships with the module, exactly like the contract (§22.6): a unit that has written nothing still presents complete conditions, said to be a starting point rather than legal advice. They are written by the asset's **managers**, from that asset's own settings page — not from the configuration mode on the public page, which needs a superadmin and therefore put the one person able to write them outside the people who let the hall. That is how so many assets ended up with none, while the form still made a visitor tick « J'accepte les conditions de location » over an empty block: the renter accepted nothing, and the hash below attested to it faithfully.
...
**The renter fills in their own billing coordinates**, from their tracking page, into the very encrypted columns a manager types into by hand — one write path, and a manager keeps the right to correct them. Asked there and not on the public request form, because a visitor enquiring about a weekend has no reason to type a VAT number and most renters never need one; the block presents itself as a task while it is empty and folds away once filled. No email chases it: the confirmation already carries the tracking link and a call to action, and a dedicated message or a reminder about it is deliberately not written.
...
Storage locations are configured site-wide (Configuration > Stockage): a local disk, an S3-compatible bucket or a Google Drive folder, and **several may coexist** — adding a new destination for future albums never breaks reading old ones. An album is pinned to the location it was created in; new albums go to the one chosen in Configuration > Galerie (« emplacement des nouveaux albums »), or to the site's default when none is chosen. Animateurs never choose. An album can be **migrated** to another location as a background task: it is unavailable while the copy runs, and a failed migration leaves it working exactly as before, since the source is only released at the very last step.
...
**Capture admits exactly one exception, and it is a category, never an address.** No allowlist of addresses that really go out and no per-recipient exception: an operator who has to work out which half of the mail left the server cannot answer "what did this feature actually send?". The one exception is the **e-mail carrying a sign-in link**, and it exists because the sandbox otherwise deadlocks the very installation it is meant to make testable — armed, that e-mail lands on the sandbox page instead of in an inbox, so nobody can sign in to the site being tested, and nobody who signed in that way can come back to disarm the capture. Off by default; the operator turns it on knowingly.

The exception survives the test above because it stays answerable: **a sign-in link that goes out is filed here anyway**, badged « Remis au destinataire » in the list and on its detail page. Nothing leaves the server without leaving a row behind. Beyond that one category nothing records **which** feature sent a message — the subject and the timestamp are the identification, deliberately.
...
A state block carries the switch, its state in plain French and — **only when armed** — the sign-in-link exception as a checkbox, with the consequence of whichever state it is in written out: unticked, the warning that sign-in links now land on this page, so the operator should sign in with a password or a passkey rather than lock themselves out of the switch; ticked, that they still reach their recipient and continue to be filed here as « Remis au destinataire ». Only when armed, because with the capture off every e-mail leaves regardless and a control that changes nothing is worse than no control. **Ticking or unticking it is journaled, in both directions**, like the switch itself.

Below it, the captured messages: subject, recipient, date, size, attachment count, DKIM badge, a « Remis au destinataire » badge on the ones the exception let out, newest first, paginated, with the whole view state in the query string and no cookie. **Two searches that cost different things**: the subject is stored in clear and searched plainly; the recipient is encrypted and therefore matched **only on an exact address** — a partial address finds nothing rather than quietly scanning everything; and an opt-in third search decrypts and reads the bodies, **bounded** (200 messages) because there is no index to lean on, which the page states rather than silently stopping.
...
| Page | Menu | Colonne | Libellé | Rôle |
|---|---|---|---|---|
| Proposer et demander (`/covoiturage`) | Espace membres | Activités | Covoiturage | membre identifié |
| Organiser (`/covoiturage/organiser`) | Espace animateurs | Activités | Organiser les covoiturages | animateur (`chief`) |
