...
| Correspondances Desk | A box at the top of the page names what the last import could not match to anything the site knows — an unqualified function, or a branch the code sorts last — with what the site did instead, how many records are affected, and where to fix each one; it disappears by itself once everything is resolved, and says that the labels are also reported to the maintainer when the daily report is enabled. Map Desk functions to site roles; rename sections, set section email, and toggle section visibility across the site. Per age branch: federation logo (uploaded, falls back to a shipped default per canonical branch, else nothing) and explanation link (defaults to the Les Scouts federation page), shown on the member page's branch card (§4.2). |
