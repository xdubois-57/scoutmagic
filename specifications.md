...
- **Who counts as one voter**: the connected person (the default — one answer per email address), or the member. Per member is what a parent of several children needs; when their account is linked to more than one member, a small dialog asks whose answer is being given at the moment of voting. It offers **every member that account reaches**, the group's own first, and not only the ones this group is built from: a family of four children all reachable through one address has four answers to give, whichever sections those children are in, and offering two of them would produce a count that is quietly short. This is wider than what publishing offers on purpose — a message is signed as somebody who is here, an answer is counted for somebody the poll is asking about. The dialog **says which is which**: the members this group is built from come first, the others under a heading of their own (« Dans ce groupe » / « Hors de ce groupe »), and neither heading appears when everybody is on the same side. Without that, a parent reads four totems and cannot tell which children the question is about.
