...
- **routes**: each entry must have `path`, `controller`, `action`, `menu`, `role_min`.
  ...
  - `menu_order`: **required on any route that declares a `label`**, and `Tests\Architecture\MenuEntriesDeclareTheirPlaceTest` fails without it. It is where the entry sorts in its menu, on **one scale shared with every other entry of that menu, core pages included** — lower first. The manifest still defaults it to `100` for a route with no label, which is nothing anyone sees. Use 1–599, leaving gaps of 10; 600 and up belongs to the free-text pages a unit writes itself (`Core\Page\TextPageMenuProvider::ORDER_BASE`), which always come last. Only *dynamic* entries have a rank above the scale — `espace_animes`'s per-member pages, always first — so a module page with a low enough number now precedes a core page, which it could not before.
...
- **settings**: optional, each entry must have `key`, `type`, `label`, `description`, and may declare `default_value` and `editable` (bool, default `true`). **Changing a `default_value` in a later release reaches installed sites**: a value still equal to the previous default was never customised, and moves to the new one on the first boot of that release; a value someone changed is never touched, and neither is a `secret` setting (`Core\Config\SettingRepository::updateDefaultValue()`, ARCHITECTURE.md §8.4). So a default is a promise about never-customised values everywhere, not only on fresh installs — and one key has one declared default: register the same key from two places with two defaults and a never-customised value is dragged back and forth.
...
- `prefetch`: optional boolean, defaults to `true`. `false` keeps the page whitelisted (a visit caches it, it is served offline afterwards) but leaves it out of the installed app's pre-download at launch. Declare it on a page that is costly to render — a full roster, a page hydrating every staff — and rarely the reason the app was opened; the pre-download runs for every visitor whose role can see the page, so an expensive page there is paid by everyone.
...
5bis. **Publish a directory if the Chef d'Unité should be able to file mail under your objects by hand.** `Api\ReferenceDirectory` is optional: `searchReferences(string $query, int $limit)` answers with `ReferenceSuggestion`s as a person names things (« Ferme du Bois-Joli — 12–22 juillet 2027 »), and must return an exact reference typed in full as a suggestion of its own, because `/courrier` only ever files towards a reference your search returned; `referenceUrl()` says where the object lives, and every rattachement on the message page links there. Without it your associations are still shown and your propositions still confirmed — the chief just cannot pick one of your objects freely.
