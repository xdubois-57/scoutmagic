...
3. **Register the consumer in the sync handler's factory** in `public/scheduler-bootstrap.php` — the registry lives entirely on the scheduler path, built lazily with every enabled module's consumer, identically for both entry points. Registration order matters (first claim wins): a consumer that claims broadly goes last, and the factory is where that ordering is written once.
