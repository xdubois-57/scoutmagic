...
## 29. Deferred hardening (known, tracked)
...
- **Document-root-wide `post_max_size`** (`public/.user.ini`). The 2 GB ceiling is needed only by the chief-only, single-POST gallery **video** upload (multipart, buffered to disk), but `.user.ini` is per-directory and every route runs through `public/index.php`, so it also applies to `/login`, `/password-reset` and `/inscriptions`. A urlencoded body to one of those is buffered to memory before application code runs. Proper fix: a dedicated upload entry point in its own directory with its own `.user.ini` (lower the global limit), or chunked uploads so no single request body is large — entangled with the streaming work in §"Serving large files".
