...
- Six deliberate exceptions, each authenticated by something other than a session-bound token:
  ...
  - `POST /api/support/tickets` (`Modules\SupportDashboard\Controller\TicketIntakeController`, ARCHITECTURE.md §8.49ter) — the support-ticket intake on the receiving installation, and a machine-to-machine call for the same reason as the statistics intake above. Authenticated by **the same identity**: the installation's own bearer secret, verified with `password_verify()` against the `password_hash()` the statistics intake stored, so there is no second credential to lose or rotate. There is no hard-coded dummy hash: since roadmap IT-24 an unknown installation id is *registered* under trust-on-first-use rather than compared against a bcrypt literal committed to this repository, and the refusal a caller sees is the same bare status whatever the reason, so it can never learn which installation ids this receiver knows. Absent, malformed and wrong credentials are one answer: **403 with a bare status and a `security` journal entry naming the source address and nothing else** — never the installation id somebody *tried*, which would file a failed attempt under a unit that may have had nothing to do with it. **Every other refusal answers 200**, deliberately: a client that receives a non-2xx retries, and there is nothing to retry about an unknown category or a spent hourly quota, while a ticket accepted and then retried is a ticket filed twice — the same reasoning as the GitHub webhook. The endpoint refuses cleartext transport, caps the body at 32 KB before parsing it, rate-limits per installation, and validates the category against a closed list declared in code. The description and the contact address are personal data: encrypted `BLOB`s decrypted only in `Repository\SupportTicketRepository`, with a blind index on the address, and **neither ever appears in a journal entry** — an entry carries the installation, the category and the ticket id.
  - `POST /api/support/tickets/{reference}/archive` (`Modules\SupportDashboard\Controller\TicketIntakeController`, ARCHITECTURE.md §8.48/§8.49ter) — the diagnostic archive of a ticket that already exists, on its own call because an upload that times out must not take the report with it. Same bearer identity as the ticket route above, verified against the installation the *ticket* belongs to; an unknown reference answers the same 403 as a bad secret, so a caller cannot discover which references exist by watching which refusal comes back. It refuses cleartext transport, caps the body at 60 MB **before** anything else touches it and refuses explicitly rather than truncating (half an archive is not a diagnosis), and is idempotent: a retry after a timeout that actually succeeded attaches no second copy. The archive is stored through `Core\File\EncryptedFileStorageService` — encrypted at rest, `role_min: superadmin`, reachable only through `/files/{id}` — which is the same floor it had on the installation that produced it.
  - `POST /api/support/mail-probes` (`Modules\SupportDashboard\Controller\TicketIntakeController`, ARCHITECTURE.md §8.49quater) — where an installation asks for a correlation key and the addresses to send a diagnostic probe to. Same bearer identity again, and **authentication here is not a formality**: the answer is a list of this receiver's own mailbox addresses, so an open route would hand them to anybody who asked. Every failure — no bearer, wrong secret, unknown installation, no installation named, cleartext transport — answers the identical bare `rejected` with a 403. A `POST` although it reads like a lookup, for two reasons: it writes a row per mailbox, and the identity travels in a body rather than in a query string that proxies and access logs keep. **Deliberately not rate-limited.** There was one run an hour per installation, and its cost fell exactly where the probe is most needed: a unit reports « mes e-mails ne partent pas » *because* they have been pressing send, and the probe that would answer them was refused for being the second one that afternoon. Since a support ticket now always carries a probe, that limit silently dropped the evidence attached to a bug report — one message saved, the diagnosis lost. The bearer identity above is what stands between a stranger and this receiver's mailbox addresses, and it is the guard that matters; the amplification a valid, authenticated installation can cause is bounded by mailboxes this receiver owns and reads. The mailbox address and the header findings it later stores are encrypted `BLOB`s decrypted only in `Repository\SupportMailProbeRepository`; the journal counts mailboxes and names none of them.
...
- Beyond the `role_min` floor, a file may be narrowed by ownership: `files.owner_member_id` (the session must be linked to that member) or the generic `files.owner_type`/`files.owner_id` registry (`Core\File\FileOwnershipCheckerInterface`, ARCHITECTURE.md §8.3). Both are **fail-closed** — an `owner_type` with no registered checker is denied, never allowed by default. A checker can only narrow what `role_min` already permits, never widen it, and `owner_type` values must be unique across checkers (the first `supports()` match wins). A module's checker is wired into the composition root only while that module is enabled, and the guard is built after every module block precisely so a missing checker can never silently become "no check".
- **`files.owner_member_id` has a Staff d'Unité bypass, and this line used to say the opposite.** The rule here was that neither ownership mechanism had a chief/admin bypass. That is still true of the `owner_type` registry; it is no longer true of `owner_member_id`, where `FileAccessGuard::STAFF_BYPASS_ROLE` (`admin`) reads any owner-scoped file. It was withdrawn on purpose so a chef d'unité can open one private document from a member's sheet and send it again when a family says they received nothing (ARCHITECTURE.md §8.3 and §8.86 carry the full reasoning). **Accepted risk, stated rather than discovered**: the mechanism is not compartmentable, so this opens every file that will ever be owner-scoped, not only certificates — today `member_documents` are the only ones. Three bounds: it stops at `admin` (never `chief` — an animateur de section reads none of it); it lifts the ownership half only, the `role_min` floor still being checked first and independently; and every such opening is journaled at `security` level (`member_document_opened_by_staff`, identifiers only), as is every resend (`member_document_resent`). The audit trail is the compensation for the wall, which is why it is `security` and not `info`.
...
- **The diagnostic support package** (ARCHITECTURE.md §8.48) is treated as the most sensitive artefact this codebase produces on demand, because it deliberately contains `phpinfo()` output, server logs and filesystem diagnostics: written under `storage/core/support/`, **encrypted at rest** via `Core\File\EncryptedFileStorageService`, registered with `role_min: 'superadmin'`, and reachable only through `/files/{id}`. Exactly one is kept (generating replaces the previous file and its `FileRecord`) and a daily task deletes it seven days after generation, downloaded or not. It is **never transmitted automatically** — no scheduled upload, no email, no pre-filled `mailto:` with an attachment, nothing that happens because a task ran. Since roadmap IT-26 it **can** be transmitted, and only ever by an explicit act: an administrator attaches it to a support ticket from Configuration > Support, after the page has listed what the archive contains in French with its size, and after ticking an acknowledgement that is verified **server-side** rather than in the browser. It travels on its own call, separate from the ticket, so a failed upload costs no report; it is stored on the receiver through the same `Core\File\EncryptedFileStorageService` with the same `role_min: 'superadmin'` and the same `/files/{id}`-only reachability it had here, because the receiver must not be the weakest place a unit's diagnostics ever sit; and the act is journaled at `security` level on the sending installation. Everything below still holds of what is *in* it. Every reason and note written into its `collection-status.json` is scrubbed of every known secret **before** any whitespace normalisation, so a collector's exception quoting a credential cannot smuggle one into an archive destined for email — the needles include the database and SMTP passwords, which may contain a space, and collapsing whitespace first was enough to stop the needle matching. The collection is also bounded as a whole (25 log files / 8 MB, 5 000 `storage/` entries, each truncation stated in the archive), because the guarantee this feature actually makes is that a package is *always* produced, and an unbounded read on a busy host exhausts `memory_limit` and produces none.
...
- An inbound message's attachment carries `files.owner_type = 'inbound_message'` / `owner_id = inbound_messages.id`, and that ownership — not the `role_min` floor — is what partitions it. **This closed a real hole**: attachments were written with `role_min = 'intendant'` and no owner at all, so any intendant could read any attachment of any watched mailbox by walking `/files/{id}` — a rental contract, a camp's medical form, an invoice from a box they have nothing to do with. `role_min` never was the partition; it only ever said "not the public", and the floor deliberately **stays** `intendant` (a Finance intendant's legitimate access would break if it were raised — the floor is a belt, the ownership is the lock). `Modules\InboundMail\Service\InboundMessageAccessRegistry` resolves it by asking the consumers the message is actually associated with (`Api\MessageConsumerInterface::canRead()`, answering with the same rule their own screens use) and grants as soon as one says yes; a consumer that throws is a refusal, and an association whose module is disabled grants nothing — disabling a module never hands its data to somebody else. Two answers are decided rather than delegated: a message **no consumer is associated with** is readable by the Chef d'Unité alone (that is the general mailbox's archive), and the Chef d'Unité reads any of it, since they hold that mailbox by design. **This creates no exception to `/files/{id}`**: there is no side route, no signed link and no bypass — the request goes through `FileAccessGuard` exactly like every other file, `role_min` is enforced first and independently, and the checker can only narrow. Attachments stored before the ownership existed are backfilled from the composition root (`InboundMailService::backfillAttachmentOwners()`, guarded by `inbound_mail_attachment_owners_migrated`); a file shared by several messages through deduplication has its ownership handed to a message that still holds it when the named one is destroyed, so it never points at a message that no longer exists. **A refusal is journalled as `file_access_denied` with the file id and the IP, and nothing else** — never the subject, the sender or the attachment's filename, which would put in the journal exactly what the refusal was protecting.
...
- **A customised automatic e-mail is data, never a template** (`Core\Mail\Template\EmailTemplateRenderer`, ARCHITECTURE.md §8.7bis). The subject and body a superadmin writes on Configuration > E-mails are stored sanitized like any other rich text, and on the way out each declared `{{ variable }}` is replaced by **string substitution** — the stored text is never handed to Twig, in any form: no `createTemplate()`, no sandbox, no `include`. Twig syntax in it comes back out literally. This is the whole reason the feature is safe to offer at all: a page that could define a Twig template is a page that can run code, and a sandbox is a list of things somebody has to keep remembering to forbid. The frame around the body (`email/base.html.twig` — header, footer, the unit's name) stays code and is not customisable, and the four **authentication** e-mails (magic link, password reset, the two address confirmations) are `editable: false`, refused **on the server** rather than merely absent from the page: an administrator who broke one would lock themselves and everybody else out with no way back in.
...
- **The federal cotisations page** (`fees_federal_scale_url`, admin, ARCHITECTURE.md §8.75) belongs to this family too, and is the one member of it whose *body* is read rather than just POSTed to. `Modules\Fees\Service\FederalScaleLookupService::fetchPage()` runs `SsrfUrlValidator::assertPublicHttpsUrl()` **on use**, on every click, never only when the setting was saved; it then makes one plain `GET` with a bounded timeout, a 2 MiB read cap, and **no redirect followed at all** — the scraper's hop-by-hop re-validation (above) is deliberately not reimplemented here, because a federation page that moved is one setting to correct, not a chain to chase, and refusing is both safer and more honest than silently reading somewhere else. A non-200 answer, a rejected URL and a network failure are the same single outcome outward («la page n'a pas pu être consultée»), and nothing about the target beyond the configured URL itself reaches the journal. This is not a second scraper in the sense the first bullet forbids: no member-supplied URL ever reaches it, the value is admin-typed on the settings page, and it fetches exactly one configured document.
...
### 18bis. Sending a third party's web page to an AI provider

`Modules\Fees\Service\FederalScaleLookupService` (« Chercher les montants » on the barème, ARCHITECTURE.md §8.75) is the first feature that fetches a page **this project does not control** and hands it to a model whose answer then lands in a form field. That is a prompt-injection surface, not a data-protection one: the page carries no personal data, but it can carry a sentence written to be read as an instruction. The contract, in the order it actually runs:

- **The page is the user prompt, never the system prompt.** Every instruction lives in the system prompt; the fetched text arrives as data, introduced as *contenu à analyser* and fenced between two markers, with the system prompt stating in as many words that a sentence in it which looks like a consigne is to be ignored. The two markers are stripped out of the page's own text first, so a page printing one cannot close the fence early and have what follows read as though the site had written it. This is §18's "the member's text is the prompt" rule applied to a page instead of a member.
- **The answer is a shape, not a message.** A JSON Schema goes out with the request and the reply is decoded as a JSON object here — fences, preamble and trailing prose stripped, anything that is not an object refused. No field of the answer is ever executed, concatenated into a second prompt, rendered as HTML, or used to build a URL.
- **Every value is re-derived, never taken.** Three amounts must parse as a number inside a plausible cotisation range (1 € – 500 €), so a crafted page cannot make the screen propose an absurd figure; the year must normalise to `YYYY-YYYY` and equal the scout year the screen is about, which is what makes the page's two coexisting years («COTISATIONS 2025-2026» and «COTISATIONS 2026-2027») a checked fact rather than a coin toss. Anything else is a refusal with nothing pre-filled.
- **The only untrusted string that reaches the screen is the normalised year**, and it reaches it only after that regex has rebuilt it. The model's prose, a page excerpt and the connector's own error text are never quoted back — every message on this path is a fixed French sentence written in `Value\FederalScaleLookup`.
- **Nothing is ever written from the answer.** The service holds no repository and cannot reach the database. The proposal crosses one redirect in the asker's own session (`Support\SuggestedScale`, the `RejectedDraft` shape) to pre-fill three form fields; the only write is the chef d'unité's own « Enregistrer le barème », through the same validated POST that has always existed.
- **The screen says what the figures rest on.** The URL that was read and the year that was found are displayed with the proposal, because a number a model produced and nobody can trace is worse than an empty field.
...
`public/cron.php` is the scheduler's command-line entry point, and since a real crontab became a verified requirement (`ARCHITECTURE.md` §8.5) the ONLY thing that runs a scheduler pass. It runs privileged maintenance work with no session and no RBAC, so it must never be reachable over HTTP.
...
- **There used to be a seventh, and its removal is worth recording**: `POST /api/scheduler/continue` (`Core\Http\Controller\SchedulerContinuationController`) — the hop the scheduler emitted onto its own site so the queue would drain without a crontab. It was public, session-free and CSRF-free, authenticated by a shared secret kept in `secrets.enc`; it was also **the one path `Maintenance\MaintenanceGate` never blocked**, an exemption that existed because the gate answered that hop with the « mise à jour en cours » page, gating on the very row the hop came to advance. A real crontab is now a requirement, verified before a first install can complete (ARCHITECTURE.md §8.5), so the mechanism, the route, the secret and the gate exemption are all gone — the gate has no exemptions at all today, and this installation makes no HTTP request to itself. A route removed is one fewer unauthenticated surface; a gate with no exemptions is one fewer argument about the next one.
