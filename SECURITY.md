...
- Seven deliberate exceptions, each authenticated by something other than a session-bound token:
  ...
  - `POST /api/scheduler/continue` (`Core\Http\Controller\SchedulerContinuationController`, ARCHITECTURE.md §8.5) — the poor-man's-cron continuation hop. The caller is this installation's **own** PHP process, which has no session for the guard to reason about, and a shared secret is the whole authorisation. That secret lives in `secrets.enc`, never in `settings`, and is generated on the first request that finds none — unlike the webhook's, there is no admin action to hang it off. A bad or absent secret gets the same 403 and the same `security` journal entry, with the presented secret never recorded. It is also **the one path `Maintenance\MaintenanceGate` never blocks** (`Scheduler\SchedulerContinuationRoute::PATH`), which is what lets an update finish at all: the gate ran before routing and answered this hop with the « mise à jour en cours » page, gating on the very row the hop came to advance. That exemption costs nothing here — the gate is an availability gate, not a security boundary, and the shared secret still decides whether the caller gets past the controller.
