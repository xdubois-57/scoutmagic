...
The statistics receiver (ARCHITECTURE.md §8.45) keeps each reporting installation's **instance URL** and **installation id** as plain columns. Neither identifies a natural person: a scout unit is an association, the URL is already public, and the installation id is opaque random bytes derived from nothing about anyone. Both are needed in clear for the dashboard to filter, sort and search across installations — encrypting them would force either a blind index per searchable field or full-table decryption on every page load, buying nothing. The reports themselves carry no member data at all (§8.43), so there is nothing else in that table to protect.
