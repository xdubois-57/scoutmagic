...
final class RemoteBackupController extends AbstractController
{
    ...
    /**
     * POST — records that the phrase in force has been copied off this
     * server.
     *
     * **A statement, not a measurement, and nothing here can check it.**
     * That is not a weakness of the design; it is the only shape
     * available. The copy that matters lives in a password manager or on
     * paper in somebody's house, and no code on this server can see it.
     * What the site can do is stop pretending the question was never
     * asked: until somebody answers, {@see \Core\Alert\Check\RemotePassphraseNotedCheck}
     * says so on the attention page, and archives go on leaving with a
     * key that exists in one place.
     *
     * **Revealing is deliberately not confirming.** Making the reveal
     * route record this would clear the warning for an administrator who
     * opened the screen to look at the generation number, which is the
     * exact false negative this exists to remove.
     *
     * The confirmation is stamped with the generation, so a later
     * regeneration brings the warning back on its own.
     *
     * @param array<string, string> $params
     */
    public function confirmPassphraseNoted(Request $request, array $params): Response
    {
        ...
        $this->journalService->log(
            'core',
            'remote_backup_passphrase_noted',
            'security',
            'Phrase de passe des sauvegardes hors site déclarée recopiée',
            ['generation' => $this->passphrase->generation()],
            AuthSession::getUserAccountId()
        );
        FlashMessage::set(
            'success',
            'Noté. Si vous régénérez la phrase, il faudra recopier la nouvelle : celle que vous venez '
    ...
    public function regeneratePassphrase(Request $request, array $params): Response
    {
        ...
        FlashMessage::set(
            'success',
            'Nouvelle phrase de passe, à recopier hors du site à son tour. Les archives déjà envoyées ne '
            . 's\'ouvrent plus qu\'avec l\'ancienne : notez-la si vous la connaissez encore, ou supprimez-les '
            . 'du compte distant.'
        );
    ...
    public function chooseDestination(Request $request, array $params): Response
    {
        ...
        // The one moment somebody is paying attention to off-site
        // backups. The phrase is generated by the first send, in the
        // middle of the night, with nobody there — so this is where to
        // say it is coming and that it will have to be written down
        // (#496). The attention point says it again afterwards, but a
        // sentence here is what stops the question from ever arising.
        FlashMessage::set(
            'success',
            sprintf(
                'Les sauvegardes hors site partiront vers « %s ». Le premier envoi créera une phrase de '
                . 'passe qui les chiffre toutes : il faudra la recopier hors du site, sans quoi elles '
                . 'seront illisibles le jour où ce serveur ne répondra plus.',
                $location->label
            )
        );
